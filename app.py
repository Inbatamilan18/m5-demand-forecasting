...
def load(mode: str):
    fc_p = C.OUT / f"forecast_{mode}.parquet"
    if not fc_p.exists():
        return None
    fc = pd.read_parquet(fc_p)
    hier = pd.read_parquet(C.OUT / f"hierarchy_{mode}.parquet") \
        if (C.OUT / f"hierarchy_{mode}.parquet").exists() else None
    mets = json.loads((C.OUT / f"metrics_{mode}.json").read_text()) \
        if (C.OUT / f"metrics_{mode}.json").exists() else None
    meta = json.loads((C.OUT / f"train_meta_{mode}.json").read_text()) \
        if (C.OUT / f"train_meta_{mode}.json").exists() else {}
    imp_p = C.OUT / f"feature_importance_{mode}.csv"
...
@st.cache_data(show_spinner=False)
def load_calendar():
    """calendar.csv, for the event and SNAP overlays."""
...
def load_prices_for(items: tuple[str, ...], stores: tuple[str, ...]):
    """Weekly sell_price history for a small set of item/store pairs."""
...
data = load(mode)
if data is None:
    st.warning(f"No forecasts for **{mode}** yet. Run the pipeline first:\n\n"
