...
def load(mode: str):
    ...
    mets = (
        json.loads((WEB / f"metrics_{mode}.json").read_text())
        if (WEB / f"metrics_{mode}.json").exists()
        else None
    )

    meta = (
        json.loads((WEB / f"train_meta_{mode}.json").read_text())
...
with tabs[0]:
    ...
    daily = (
        f.groupby("date", as_index=False)["forecast"]
        .sum()
    )
...
with tabs[1]:
    ...
    if hier is None:
        ...
        st.info(
            "Run `python -m src.evaluate` to build hierarchy aggregates."
        )
        ...
        st.caption(
            "Forecasts are produced at item×store level and summed "
...
with tabs[2]:
    ...
    if mets is None:
        ...
        st.info(
            "This window has no ground truth, so accuracy cannot be "
...
with tabs[3]:
    ...
    st.caption(
        "The brief asks the model to use price, promotion and holiday "
    ...
    pr = load_prices_for(
        tuple(top_items["item_id"].unique()[:40]),
    ...
    st.caption(
        "Price enters the model as three features: absolute price, "
