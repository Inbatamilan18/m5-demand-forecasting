...
# deps first so this layer caches between rebuilds
COPY requirements-web.txt .
RUN pip install --no-cache-dir -r requirements-web.txt

# app code + the precomputed data bundle
...
CMD streamlit run app.py \
  # Keep memory low on small instances: no file watcher, no usage stats,
# and cap the upload buffer.
CMD streamlit run app.py \
    --server.port=${PORT} \
